...
- Line-synchronized sources highlight the active line as one unit; intra-line color progression is rendered only when the lyric document contains real word timing. Marquee travel remains tied to line timing, not to a synthetic per-character clock.
...
- The selected Floating Bar direction is Option 1 “Floating Deck”, not a resized Dynamic Island and not a height-derived capsule.
- The independent window keeps a fixed 28pt continuous radius. The upper tier contains artwork/identity, bilingual lyric context, and transport; the lower 58pt rail contains waveform, progress/time, volume, repeat, mode, and settings.
- The deck defaults to 980 × 220pt, clamps at 760 × 190pt, and stores its redesigned v2 width/height separately from rejected legacy capsule sizes.
- Floating Deck changes must not alter compact or expanded Mac Island geometry.
